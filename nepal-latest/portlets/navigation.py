...
class NavigationPortletRenderer(BaseRenderer):
    ...
    def createNavTree(self):
        ...
        # We only want the items from the nav tree for which the current user has
        ...
        if "children" in data:
            ...
            # Find the "Patients" and "Clients" items
            patients_item = next((item for item in children if item.get("id") == "patients"), None)
            
            # Remove "Patients" and "Clients" from their current positions
            children = [item for item in children if item.get("id") not in ["patients"]]
            
            if patients_item:
                children.insert(0, patients_item)

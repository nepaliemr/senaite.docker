...
class ReportView(Base):
    ...
    FOOTER_NEPAL_TEMPLATE = PT("templates/footer_nepal.pt")
    CUSTOM_CSS_TEMPLATE = PT("templates/custom_css.pt")
    INFO_NEPAL_TEMPLATE = PT("templates/info_nepal.pt")
    PATIENT_INFO_TEMPLATE = PT("templates/patient_info.pt")
    QR_CODE_TEMPLATE = PT("templates/qr_code.pt")
    SIGNATURE_TEMPLATE_NEPAL = PT("templates/signatures_nepal.pt")
    PUBLISHER_TEMPLATE = PT("templates/publisher.pt")
    ...
    def render_patient_info(self, context, **kw):
        return self.PATIENT_INFO_TEMPLATE(context, **kw)
    
    def render_qr_code(self, context, **kw):
        return self.QR_CODE_TEMPLATE(context, **kw)
    ...
    def render_signatures_nepal(self, context, **kw):
        return self.SIGNATURE_TEMPLATE_NEPAL(context, **kw)
    ...
    def render_footer_nepal(self, context, **kw):
        return self.FOOTER_NEPAL_TEMPLATE(context, **kw)
    
    def render_publisher(self, context, **kw):
        return self.PUBLISHER_TEMPLATE(context, **kw)
    ...
    @property
    def current_user_for_report(self):
        user = api.get_current_user()
        # XXX: we're missing here LDAP properties!
        #      needs to be fixed in the API.
        properties = api.get_user_properties(user)
    ...
    def get_report_variable_range(self, analysis, report_only=True):
        items = []
        obj = api.get_object(analysis)
        interim_fields = obj.getInterimFields() or []
        for interim_field in interim_fields:

...
class SuperModel(BaseModel):
    ...
    def get_formatted_uncertainty(self, analysis):
        ...
        """
        uncertainty = format_uncertainty(analysis.instance, decimalmark=self.decimal_mark,sciformat=self.scientific_notation)

        return "[&plusmn; {}]".format(uncertainty)
    
    def get_formatted_range(self, analysis):
        """Return formatted range 60-70
        """
        range = analysis.getResultsRange()

        # get the min operator
        min_operator = range.get("min_operator") or ""
        min_operator = MIN_OPERATORS.getValue(min_operator, default=">")

        # get the max operator
        max_operator = range.get("max_operator") or ""
        max_operator = MAX_OPERATORS.getValue(max_operator, default="<")

        fs = ''

        if range.get('min', None) and range.get('max', None):
            fs = '%s - %s' % (range['min'], range['max'])
        elif range.get('min', None):
            fs = '%s %s' % (min_operator, range['min'])
        elif range.get('max', None):
            fs = '%s %s' % (max_operator, range['max'])

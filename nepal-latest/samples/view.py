...
class SamplesView(ListingView):
    ...
    def folderitem(self, obj, item, index):
        ...
        item["getDateSampled"] = self.convert_timestamp(obj.getDateSampled)
        item["getDateReceived"] = self.convert_timestamp(obj.getDateReceived)
    ...
    def convert_timestamp(self, timestamp):
        # Check if timestamp is None at the start
        if timestamp is None:
            return None  # Return None or an empty string, e.g., ""
        
        # If it's a Zope DateTime object
        if isinstance(timestamp, ZopeDateTime):
            dt = timestamp.asdatetime()
        elif isinstance(timestamp, datetime):
            dt = timestamp
        else:
            # Assume it's a string and parse accordingly
            try:
                # Check if the timestamp is None after the initial check as well
                if timestamp is None:
                    return None
                dt = datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S%z")
            except ValueError:
                # Fallback for timestamps without timezone information
                dt = datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S")
        
        # Format the datetime object to "YYYY-MM-DD HH:MM"

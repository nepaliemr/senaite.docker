...
def find_analysis_requests_by_medical_record_number(medical_record_number):
    """
    Find all analysis requests with the same hospital lab number.
    
    Args:
        medical_record_number: The hospital lab number to search for
        
    Returns:
        list: List of analysis request objects with the same hospital lab number
    """
   
    catalog = api.get_tool('senaite_catalog_sample')

    medical_record_number = [medical_record_number.get('value', 'No value found')]

    # Retrieve all samples that match the specified Client Order Number
    brains = catalog(
    ...
    for brain in brains:
        try:
            ...
            print("Error fetching sample object:", e)
...
def find_latest_id():
    ...
    """
    Find the latest ID for the analysis request
    """

    catalog = api.get_tool('senaite_catalog_sample')

    # Retrieve sample that has the highest serial number
    brains = catalog(
        portal_type='AnalysisRequest',
        SerialId={'query': '*', 'range': 'max'}
    )
    ...
    for brain in brains:
        try:
            sample = brain.getObject()
            sample_ids.append(sample.getSerialId())
        except Exception as e:
            print("Error fetching sample object:", e)

    #get biggest number from the list
    if sample_ids:


        sample_ids = [id for id in sample_ids if isinstance(id, int) and id != '']

        print("sample_ids: ", sample_ids)
        return max(sample_ids) if sample_ids else None
...
def after_receive(analysis_request):
    ...
    #getlast 2 digits of the year
    year = datetime.now().strftime("%y")

    #get 2digits of the month
    month = datetime.now().strftime("%m")

    #get 2digits of the day
    #day = datetime.now().strftime("%d")

    # Get the serial number utility
    serial_number = find_latest_id() 
    ...
    # formatted_id = serial_number.zfill(5)

    lab_number = "LAB{}-{}-{:05d}".format(year, month, serial_number)

    medical_record_number = analysis_request.getMedicalRecordNumber()

    existing_requests = find_analysis_requests_by_medical_record_number(medical_record_number)

    # update the lab number for all the analysis requests with the same hospital number
    for request in existing_requests:
        if not request.getLabNumber():
            request.setLabNumber(lab_number)
            request.setSerialId(serial_number)
